...
| 16 | [The compensating reversal](16-compensating-reversal.md) | 6k | **Written, not yet run** - the implementation is complete and unit-tested, the stack was not available, and no numbers have been invented to fill the gap. The design argument is the content: the reversal cancels the **authorization**, not the capture, because the capture legs are the ones that never posted - reversing those instead would credit `settlement:card-network` for funds it never sent and keep `SUM(amount_minor)` at exactly zero while two accounts are wrong, which is the *second* time in two phases that invariant has been structurally unable to see a real error. The DLT handler compensates only when `state == CAPTURED && !hasEntryFor(eventId)`, because a capture can dead-letter with its legs already posted when the **webhook** is what failed - reversing there takes back money the ledger says is owed, to fix somebody else's 502. And arm B replays the DLQ *after* the reversal, which is a person using the phase-6f feature correctly: the event genuinely never posted, so every duplicate defence says go ahead, and only a `reversed_capture` tombstone stops it |
