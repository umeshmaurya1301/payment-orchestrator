...
| 16 | [The compensating reversal](16-compensating-reversal.md) | 6k | A capture the ledger could not record is **given back**. At p=1.0 all 8 walk 5s+1m+10m into the DLQ - it sits flat for ten minutes then takes all eight at **630s** - and each produces a compensation that reverses it at the provider: `MERCHANT_REVERSAL -33,600` against `CLEARING_REVERSAL +33,600`, four legs netting to zero, 8 tombstones, imbalance 0, drift 0. The reversal cancels the **authorization**, not the capture, because the capture legs are the ones that never posted - reversing those would credit `settlement:card-network` for funds it never sent and keep `SUM(amount_minor)` at exactly zero while two accounts are wrong. Replaying the DLQ afterwards changes nothing, and that takes a tombstone: the replayed captures have never been posted, so every duplicate defence says go ahead. Arm C is what makes the rest mean something - a capture whose ledger write SUCCEEDED and whose **webhook** dead-lettered is skipped, not reversed. **Three runs were needed and neither failure was the saga**: the three routed providers build to their own images and were still on pre-6k code without `/reverse`, and the script asserted immediately after disarming the seam while the reversal events were still serving a 1-minute tier - repeating a lesson experiment 15 had already written down |
| 17 | [The graceful drain](17-graceful-drain.md) | 7i | Six services declared `server.shutdown: graceful` and **none could ever have completed one**: Spring's 35s drain against Docker's default 10s grace period, a bug living in the gap between two individually reasonable files. One run, identical load, one flag apart - `-t 45` exits **143 clean** after a 25s drain with **0 stranded**; `-t 10` exits **137 SIGKILL** at 11s leaving **36 payments in AUTHORIZING**, each a card that may have been charged with nothing in the system that will ever find out. But the first run of this drill passed both arms at zero and was worthless: against a 200ms provider the burst finishes inside ten seconds, so SIGKILL landed on an empty pipe - the mechanism fired and there was no cargo. Slowing the provider to 18s made arm B falsifiable, and the two arms now assert **opposite** things. Before any of that the preflight refused to run at all, reading a compose label Docker does not emit while the container's actual StopTimeout was 45 |
| 18 | [Pool starvation](18-pool-starvation.md) | 7 | 800ms in front of MySQL and the bottleneck **relocates rather than disappears**: throughput 1,860 -> 113 payments in 45s, idle connections to zero, active pinned at exactly 20/20, and `hikaricp_connections_pending` climbing 11 -> 25 -> 42 -> **74**. The finding is which instrument shows it: `jvm_threads_live_threads` reads 75 healthy and **69-71 under starvation** - it goes slightly DOWN, because the JVM counts platform threads and everything piling up is virtual. A reader watching thread count sees a system getting quieter at the moment it stops being able to work. Migrating to virtual threads moved the evidence as well as the queue. And the trap the phase names: this measures a 20-connection pool behind a slow database, **not** virtual threads |
| 19 | [Virtual vs platform](19-virtual-vs-platform.md) | 7 | **Throughput is the same either way** - 2,141 vs 2,289 payments, 6% apart and the sign changed between runs - while the same work costs **3.5x fewer OS threads** (80 vs 276). Threads are never the constraint: three attempts to build a thread-bound workload each hit a different bound first - the bulkhead (117 rejected), then the load generator itself (both arms completing *exactly* 1200 = 400 x 3 waves, the generator's ceiling showing through identically in both arms), then something at ~68 in flight still unidentified. The benchmark originally asserted `virtual > platform` and failed three runs against a system behaving correctly - an expectation with an `if` around it. The honest case for virtual threads here is memory, not speed |
| 20 | [The recon that could not conclude](20-unknown-resolution.md) | 8a | 11,601 UNKNOWN payments, oldest **4.3 days**, and the poller resolved **none of them**: 250 polls, 250 inconclusive, all four providers `silent` while the same reference answered a direct curl with HTTP 200 in 2ms. Three stacked bugs, none of which produced an error. `askEveryone` **discarded `subtask.exception()`** - the class that exists to tell 'said no' from 'said nothing' was silent about its own silence. The adapter deserialized a **list** endpoint into a flat record, so Jackson's failure became `ProviderUnavailableException` and a provider answering correctly was recorded as unreachable. And `RedisLock` had **never been created in any service** - `@ConditionalOnBean` is a registration-order check, so phase 7h's lock never existed at runtime. The resolver's caution is what made the bug both survivable and invisible: concluding FAILED from silence would have been wrong and obvious |
| 21 | [What three indexes were worth](21-index-benchmark.md) | 8 | On 131,585 real payments: a `merchant_reference` lookup goes from **131,585 rows examined to 1**, 24.000ms to 0.015ms. A covering index reads the same 49,192 rows **4.7x faster** by never touching them - `Using where; Using index` against `Using index condition`. And the deliberately bad index is worse than predicted: MySQL does **not** ignore an index on a single-valued column, it **picks it** and scans the whole table through it at 21.8ms against the table scan's 24.0ms, because the index is narrower than the row. The plan names an index, the timing is a hair faster, and every signal a reviewer checks says it works - while it is pure write cost. Selectivity, not usage, is what makes a column worth indexing. Two bugs in the benchmark pointed the same way: Q2 measured a full scan twice and its assertion passed on a substring |
| 22 | [The three mismatch classes](22-reconciliation.md) | 8 | Reconciliation is the only check that can see what experiment 15 proved the ledger cannot: a **paymentId this system never issued, 9,900 minor**, present in the provider's file and absent from the books. No internal invariant finds it - double-entry, `drift()` and the convergence assertion are all computed over our own tables and all stay green. Defects are **planted**, one per class, so the job is measured on finding precisely those: 1 double charge, 1 amount mismatch, 500 unsettled - and the matched line produced no mismatch of any kind. 43ms over 28,668 journal entries, fast only because the `$lookup` foreign field is indexed. **The class that matters has the fewest rows**: 500 benign against 1 that is money, so a threshold on total mismatches is permanently dominated by timing noise |
| 23 | [REST vs gRPC](23-rest-vs-grpc.md) | 9a | gRPC moves **13% more payments at 17% lower p95 for the same CPU** (12,580 vs 14,141; 156.7ms vs 129.8ms), reproduced across two runs agreeing to within 1%, with `AuthorizeRequest` 48% smaller on the wire and two thirds of that saving being field names alone. The headline is the process, not the number: **the first two runs passed while measuring a rate limiter**. They put the arms 0.1% apart, which read as a clean negative result confirming experiment 19, and was actually `merchant-per-sec: 50` rejecting 99.5% of requests in a millisecond - so the p95 being compared was the p95 of *rejections*, identical on both arms by construction. Same failure as experiment 19's load generator in a new costume: two arms agreeing suspiciously exactly is the signature of a shared bottleneck upstream of what is under test, not of a null result. A separate run of the broken script also showed a 27% p95 gap that vanished entirely on re-run |
| 24 | [Rotating a credential](24-key-rotation.md) | 9b | **53 requests across a live rotation, 0 rejected** - issue, retire, client switchover and expiry, with traffic running throughout. The criterion said "hashed at rest, rotation demonstrated" and the first half was already true since phase 1, which would have made ticking it defensible and wrong: hashing defends against someone reading the table, and does nothing about the key that leaks elsewhere and is never replaced because replacing it is an outage. Two findings the build forced. **Expiry must be enforced on the read path** - a job that flips RETIRING to REVOKED is the kind nobody notices has stopped, and its failure is silent because everything keeps working with twice the live credentials; the run asserts the row still says `RETIRING` while the key is refused. And `last_used_at` - the evidence that makes revocation an observation rather than a guess - puts a write on the auth path, so it is throttled to one per key per minute, which means **its own staleness sets a floor on how long "quiet" must be observed before it means anything** |
| 25 | [Who read the card](25-vault-audit.md) | 9c | **40 authorizations, 40 audit rows** - equality, not "at least one", because a log holding some of the reads is not an audit trail. Encryption answers "can a thief read this table" and never answers the auditor's first question: a service authorised to detokenize once per authorization, doing it a million times on a Tuesday night, was invisible to every control in this project. Six access attempts denied by MySQL with ERROR 1142, including the three that matter - the connector cannot erase, rewrite, or even **read** its own trail, and `vault_auditor` has no grant on the cards at all. Fail-closed turned four connector tests to 500 the instant it was enabled, because the test vault had no audit table: no card is disclosed when the access cannot be recorded. And the script initially reported all six controls **ALLOWED** while all six were working - `set -o pipefail` plus a MySQL denial's non-zero exit outranking grep's success |
