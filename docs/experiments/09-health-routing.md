# 09 — Health-based routing (phase 5a–5b)
...
- **Failover** is deliberately absent. Choosing a provider for a *new* payment
  and re-attempting one that already went out are different decisions with
  different safety properties: failing over after a possible authorization is how
  a customer gets charged twice. The rule to implement is fail over only on
  errors that *prove* the request was never processed, never on an ambiguous
  timeout — and the ambiguous case must land in `UNKNOWN`.
